...
function ShowCampaignDetails() {
  ...
  const makeSecondRequest = (id) => {
    return axios.post("http://localhost:8000/api/v1/creator/get-campaign-details", {
        userId: id, campaignId: campaignId });
  };

  const makeThirdRequest = () => {
    return axios.post("http://localhost:8000/api/v1/creator/campaign/check-shown-interest", {
